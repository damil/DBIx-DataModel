...
use constant N_DBI_MOCK_TESTS => 105;
...
SKIP: {
  ...
  $lst = HR::Employee->select(-columns  => "firstname AS fn, lastname AS ln",
                              -order_by => [qw/d_birth/]);
 ...
 SKIP: {
    ...
    my $emp2 = HR::Employee->bless_from_DB({
      emp_id => 999,
  ...
  $view->select(-columns => "lastname, dpt_name",
                -where   => {gender => 'F'});
  ...
  $view2->select(-columns => "lastname, dpt_name",
                 -where   => {gender => 'F'});
  ...
  $view3->select(-columns => "lastname, dpt_name",
                 -where   => {gender => 'F'});
  ...
  # cheat : dynamically add an auto_update column handler
  ...
  # cheat : dynamically add an auto_insert column handler
  ...
  # data for testing the ->do_after_commit() method
  my @publish_trans;
