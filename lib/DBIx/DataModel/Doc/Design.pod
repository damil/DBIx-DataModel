...
subclasses, built through class definition methods (see below).
 Most objects created during
the lifetime of the application will be either instances of those
application-specific subclasses, or instances
...
encapsulates a single database request.
...
A request to the database is encapsulated as an instance of
...
join datasource and to the schema. From that statement, data
...
L<join()|DBIx::DataModel::Doc::Reference/Statement::join()> 
creates a new statement to query one or several tables associated with
the datasource of the initial statement.  The new statement contains a
condition to restrict the results according to the primary key of the
initial table.  That statement cannot be executed yet, because the
values of the primary key are not known until we have an instance of
the initial table; but the statement can already be
L<prepare|DBIx::DataModel::Doc::Reference/prepare()>d.  Later on, we can
L<bind|DBIx::DataModel::Doc::Reference/bind()> the statement
to an instance of the initial table, and then execute it.

When applied to a B<data row> (to an instance of a Table or Join subclass),
...
values of the current object are bound to the appropriated
...
The L<select()|DBIx::DataModel::Doc::Reference/select()>  method
is defined in the 
L<DBIx::DataModel::Statement|DBIx::DataModel::Statement> class.
It performs many different things, depending on what kind
...
only works when in single-schema mode : it uses the 
...
=head3 New C<insert_into$path> method
...
the
L<set_auto_expand()|DBIx::DataModel::Doc::Reference/"set_auto_expand()">
...
Here is a list of points to improve in future versions C<DBIx::DataModel> :
 ...
 - think about reuse of statements : either it should be forbidden,
