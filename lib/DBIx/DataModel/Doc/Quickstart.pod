...
First load C<DBIx::DataModel> :
  ...
  use DBIx::DataModel -compability => 2.0;

The C<-compatibility> argument tells that we don't need any compatibility 
with versions prior to 2.0. By default, a compatibility layer for 
version 1.0 is automatically loaded, in order not to break previous
programs; but here, since we start a new application, that layer is not
needed, and therefore we will avoid polluting our symbol tables with
unnecessary code.
...
Now we can start populating the database:
  ...
  my ($bach_id, $berlioz_id, $monteverdi_id) = 
    HR->table('Employee')->insert([qw/ firstname    lastname   /],
                                  [qw/ Johann       Bach       /],
                                  [qw/ Hector       Berlioz    /],
