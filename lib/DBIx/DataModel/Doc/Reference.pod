...
L</define_table()>).  Handlers are called at each update B<and>
each insert, receiving the whole record and the table class as arguments
just like for auto_insert handlers described above. To avoid conflicts, a given
column cannot have both an auto_insert and an auto_update handler.
...
See also L<SQL::Abstract::More/"joins with USING clause instead of ON">.
...
it is a good idea to inherit from all tables mentioned in the join,
...
See also L<DBIx::DataModel::Doc::Design/"Views within the ORM">.
...
Sets or retrieves a string to be used as I<database schema name>.
...
where the database connexion has access to several schemata of similar structure.
...
to that method with a different string.
...
C<db_schema> attribute set to $string. The original Perl C<$schema> is left
...
L<DBIx::DataModel::ConnectedSource>, which is nothing but a pair of
references to a C<$meta_source> and to a C<$schema>.  That object can
then perform data selection, update or deletion.


=head3 Schema::table()

  my $connected_source = $schema->table($table_name);
...
=head3 Schema::db_table()
  ...
  my $connected_source = $schema->db_table($db_table_name);
...
=head3 Schema::join()
  ...
  my $connected_source = $schema->join($table, $path1, $path2, ...);

This method calls L</define_join()> to define or retrieve a join
subclass; then it creates a  I<connected_source> related to that 
...
=head3 Methods of connected sources

A connected source has accessor methods for its
C<meta_source> and C<schema> attributes. In addition, the
C<metadm> method is a synonym for C<meta_source>.
...
All other method calls on connected sources are just proxyed to
L</Source> methods. Here is the list :
...
=head3 Schema class methods delegated to the meta-schema
...
query, including its database connection, the various clauses for generating the
...
=head2 Constructor, accessor and proxy methods
...
This is the statement constructor; C<$datasource> is an
instance of L<DBIx::DataModel::Source>.
If present, C<%options> are delegated
to the L<refine()|/refine()> method.

Explicit calls to the statement constructor are exceptional;
the usual way to create a statement is through 
L<Source::select()|/Source::select()>.
...
Accessor for the statement's status ("new", "refined",
"sqlized", "prepared" or "executed").

=head3 sth()

Accessor for the lower-level DBI statement handle.
Automatically calls C<< $self->prepare >> if necessary.
...
The result is obtained by calling C<get_result()> on that instance,
...
=item L<flat_arrayref|DBIx::DataModel::Schema::ResultAs::Flat> (or shortened as B<flat>)
...
passed L<SQL::Abstract::More> call (for example the C<-from>
...
when in single-schema mode, class methods may be invoked
...
when in multi-schema mode, the syntax above does not work,
...
be made on an instance of L</ConnectedSource>, using this syntax :

  my $result = $schema->table('Employee')->select(...);

The C<< $schema->table('Employee') >> part returns a connected
source which will create a fake object of class C<HR::Employe>
and invoke the C<select()> method on it; technically this is a
Perl instance call, but it will be recognized as a
...
are passed as-is to the C<SQL::Abstract::More> and DBI layers, under the 
...
passed to the C<SQL::Abstract::More> layer.



=head2 Bimodal methods (can be invoked both as class and as instance methods)
...
in memory within the object, except the primary key columns.
