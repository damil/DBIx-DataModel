...
It covers
most public methods, except for a couple of peripheric classes
...
the C<$db_name> refers to a database view or to an ORM view
(a SQL fragm :aent that joins several tables) : it is probably
a good idea to declare as parents all tables that participate in
the join, so that the methods of those parents become available
...
C<DBIx::DataModel>, the special semantics attached to kind C<Composition> is :
...
permanent change (or more precisely : a change valid until 
C<db_schema()> is called again with a different argument).
...
This method should be called from within a transaction.  The coderef
...
The C<< $schema->table('Employee') >> part returns an
