...
L<DBIx::DataModel::Doc::Delta_2.0>). 
...
By contrast, the L</"SCHEMA METHODS">
...
Data is stored in I<data sources> that are accessed through
a given I<schema>; therefore, before issuing data manipulation
statements, we need to establish a relationship between
a data source and a schema : this is the job of the two 
methods below. Both create a fresh instance
of L<DBIx::DataModel::Statement>. The statement can then
perform data selection, update or deletion, as described in the next
chapter.

=head3 Schema::table()

  my $statement = $schema->table($table_name);
  my $result = $statement->select(...); # or ->insert(..), ->update(..), etc.
...
a I<table> object or I<meta_table> object, it returns a I<statement>;
however the statement delegates many methods to the table and therefore
can mimic it (actually, it works like a kind of proxy). An intermediate
statement object is needed for holding a reference to the schema : without
this important piece of information, the table has no connection to the
database and therefore cannot fetch any data.
...
=head3 Schema::join()
  ...
  my $statement = $schema->join($table, $path1, $path2, ...);
  my $result    = $statement->select(...);

This method calls L</define_join()> to define or retrieve a join
subclass; then it creates a fresh statement related to that 
join datasource and to the C<$schema>. From that statement, data
...
Besides, derivatives of the present method are defined 
in L</Statement::join()> and L</Table::join()>.
...
A I<statement> object encapsulates a request to the database.  It
...
=head3 insert()
  ...
  my @ids = $statement->insert({col1 => $val1, col2 => $val2, ...}, 
                               {...},
                               %options);
  # or
  my @ids = $statement->insert([qw/ col1   col2   .../],
                               [    $val1, $val2, ... ],
                               ...);
...
=head3 update()
  ...
  $statement->update(-set   => {col1 => $val1, ...},
                     -where => \%condition);
  $statement->update({col1 => $val1, ...});
  $statement->update(@primary_key, {col1 => $val1, ...});
...
update). When using this syntax, no C<to_DB> column handlers are
applied (no "deflation"), because data passed to the C<-set> argument
is raw data that did not transit through Perl objects.
...
For all syntaxes, the C<update> method only updates the columns received
...
=head3 delete()
  ...
  $statement->delete(-where => \%condition);
  $statement->delete({col1 => $val1, ...});
  $statement->delete(@primary_key);
...
=head3 fetch_cached()
  ...
  my $record = $statement->fetch_cached(@key_values, \%options);
...
C<< $statement->meta_source->{fetch_cached}{$dbh_addr}{$freeze_args} >>
...
C<$table_name>, forgetting the fact that the subexpression C<<
$schema->table($table_name) >> is a statement and not a table!


=head3 Statement::join()

  my $join_stmt = $source_stmt->join(qw/path1 path2 .../)->prepare;

Starting from a given statement, returns a reference to another
statement that will select a collection of data rows from tables
associated to the current meta-source, performing the appropriate
joins.  Internally this is implemented throught the C</define_join()>
method, with an additional C<-where> criteria to constrain on the
primary key(s) of the meta-source.  That statement cannot be executed
yet, because the values of the primary key are not known until we have
an row of that source; but the statement can already be
L<prepare|/prepare()>d.  Later on, we can
L<execute|/execute()> the statement by binding
it to an instance of _class> :

   my $obj          = $source_stmt->fetch(...);
...
Such methods will be called I<path methods>; their
...
of side-effects across your code : after 
  ...
  $obj->expand(some_path => (-columns => [qw/just some columns/],
                             -where   => {some_field => 'restriction'}))

further calls to C<< $obj->some_path() >> will just return
a dataset restricted according to the above criteria, instead
of a full join. To prevent that effect, you would need to 
C<< delete $obj->{some_path} >>, or to call the role
with arguments, like C<< $obj->some_path(-columns => '*') >>.
...
L</set_auto_expand> method.
If the optional argument C<$with_recursion> is true, then 
C<auto_expand> is recursively called on the expanded objects.



=head2 apply_column_handler()

  $class ->apply_column_handler($handler_name, \@objects);
  $object->apply_column_handler($handler_name);
...
=head2 has_invalid_columns

  my $invalid_columns = $obj->has_invalid_columns;
  if ($invalid_columns) {
...
implicitly creates a a statement from  the source's schema
...
calling C<Statement::delete()>.


=head2 Table::join()


Calls L</"Statement::join()"> to create a statement, and immediately
...
=head2 class methods delegated to the Statement class

When in single-schema mode, some class methods
are delegated to a statement created on the fly, 
...
Returns a flattened hash, built from C<no_update_columns> declared
in this source, in its ancestors, or in the C<$meta_schema>.
If a C<$path_name>, returns the corresponding entry in 
C<%all_path>.
