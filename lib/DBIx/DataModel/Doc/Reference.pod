...
in L<DBIx::DataModel::Statement::JDBC> and L<DBIx::DataModel::Statement::Oracle>.
...
The advantages of joins with USING a discussed in 
...
L<DBIx::DataModel::Statement::Oracle> if you work with an Oracle database.
...
L<multi-schema|DBIx::DataModel::Doc::Glossary/"multi-schema">
mode, and returns a new schema instance. Options to be passed
to the constructor are exactly the same as those described for the
L</"define_schema()"> method.
...
=head2 Manipulating schema state
...
an instance will be implicitly created. The reason for using
something else than this default might be either 
to control some specific parameters of C<SQL::Abstract::More>,
or to subclass it.
...
Internal references to C<__schema> are also removed.
...
results from such additional criteria within the C<ON> clauses
I<may be different> from a usual C<WHERE> criteria at the end
of the statement; if in doubt, read your vendor's manual and do
experiments. 
