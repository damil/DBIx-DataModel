...
The C<%options> are specified at the I<end> of the argument list; 
currently there is only one option C<-returning>, with an argument
that may be either a scalar, an arrayref or an empty hashref :
...
if this option is absent, values returned by calls to
