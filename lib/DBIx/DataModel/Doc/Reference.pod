...
A I<connected source> is basically just a pair of references to 
a C<$meta_source> and to a C<$schema>. Such objects are created explicitly
through methods L</Schema::table()>  or L</Schema::join()>; they are also
created implicitly when, in single-schema mode, class methods are called on
table subclasses.
...
Proxy method to L</"Statement::select()">

=head3 ConnectedSource::fetch()


=head3 ConnectedSource::fetch_cached()
...
Calls L</"ConnectedSource::join()"> to create a statement, and immediately
