...
For backwards compatibility, an alternative syntax can be 
expressed by position, as in :

  $records = $target->select(\@columns, \%where, \@order);

However this should be considered as deprecated; named arguments are
recommended because they are much more readable and offer a richer set
of options.
...
=item C<< -postBless => \&postBless_callback >>
